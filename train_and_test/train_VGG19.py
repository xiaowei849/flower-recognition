...
def model_load(class_num):
    # 基础模型
    base_model = tf.keras.applications.VGG19(input_shape=(img_size, img_size, 3), include_top=False, weights='imagenet')
    # 将模型的主干参数进行冻结
...
def train(train_ds, val_ds, model):
    ...
    # 保存模型
    model.save('../resources/models/VGG19.h5')
    # 输出训练用时
...
def show_loss_acc(history):
    ...
    plt.xlim(0, epochs)
    plt.savefig('../resources/results/VGG19_results.png', dpi=100)
