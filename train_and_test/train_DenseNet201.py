...
def train(train_ds, val_ds, model):
    ...
    # 保存模型
    model.save('../resources/models/DenseNet201.h5')
    # 输出训练用时
...
def show_loss_acc(history):
    ...
    plt.ylabel('Accuracy')
    plt.title('DenseNet201 Training/Validation Accuracy')
    plt.xlim(0, epochs)
    ...
    plt.xlabel('epoch')
    plt.ylabel('Loss')
    plt.title('DenseNet201 Training/Validation Loss')
    plt.xlim(0, epochs)
    plt.savefig('../resources/results/DenseNet201_results.png', dpi=100)
